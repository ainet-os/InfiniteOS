    ...
    <div class="relative p-6 bg-white z-1 dark:bg-gray-900 sm:p-0">
      ...
      >
        <div class="flex flex-col flex-1 lg:w-1/2 w-full">
          <div class="w-full max-w-md pt-10 mx-auto flex items-center justify-end">
            <LanguageSwitcher />
          </div>
              ...
              <div>
                <form @submit.prevent="handleSubmit">
                  <div class="space-y-5">
                    ...
                    <!-- Error Message -->
                    <div v-if="error" class="rounded-lg bg-danger/10 p-3 text-sm text-danger">
                      {{ error }}
                    </div>
                    <!-- Button -->
                    <div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (!response || !response.token) {
      console.error('响应中没有token:', response)
    ...
    window.dispatchEvent(new CustomEvent('session-initialize'))
